...
function generateMockDb() {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    let dailyAiTime = 0;
    const dailyProjs: Record<string, number> = {};
    const dailyLangs: Record<string, number> = {};
    ...
    for (let s = 0; s < sessionsCount; s++) {
      ...
      // Pick files edited
      const sessionFiles: Record<string, any> = {};
  ...
  sessions.forEach(s => {
    ...
    p.aiTime += s.aiTime;
    if (!p.branches.includes(s.branch)) { p.branches.push(s.branch); }
  });
